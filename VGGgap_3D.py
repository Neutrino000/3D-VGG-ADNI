...
class VGGgap(nn.Module):
    def __init__(self, batch_size, in_channels, channels, output, dropout):
        ...
        self.single_blk3 = VGG_single_blocks(self.channels*2, self.channels*2)
        ...
        self.vgg_blok3 = VGG_blocks(self.channels*8, self.channels*8, 1)
        self.gap_fc = nn.Linear(channels*8, self.output)
        # self.fc1 = nn.Linear(self.channels*8*3*3*3, 4096)
    ...
    def forward(self, x):
        ...
        x = x.view(self.batch_size, self.channels*8)
        x = self.gap_fc(x)
        # x = self.fc1(x)
        # x = self.dropout1(x)
        # x = self.fc2(x)
...
# vgg = VGG(2, 4, 32, 2)
# input_data = torch.ones(2, 4, 61, 61, 61)
